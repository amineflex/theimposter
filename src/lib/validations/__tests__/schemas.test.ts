...
describe('createRoomSchema', () => {
  it('borne le nombre de joueurs entre 3 et 12', () => {
    const payload = {
      playerName: 'Hôte',
    ...
    expect(createRoomSchema.safeParse({ ...payload, maxPlayers: 3 }).success).toBe(true)
    expect(createRoomSchema.safeParse({ ...payload, maxPlayers: 2 }).success).toBe(false)
    expect(createRoomSchema.safeParse({ ...payload, maxPlayers: 13 }).success).toBe(false)
  })
})
