...
import type { EnginePlayer, GameState, WordSet } from '@/lib/game-engine/types'
import type {
  GamePlayerRow,
  GamePlayerStatusRow,
  GameRow,
...
export class ConcurrentUpdateError extends Error {
  constructor() {
    super('La partie a déjà avancé, réessayez.')
...
export async function saveGame(
  ...
  if (finished) {
    const { data: gameRow } = await admin.from('games').select('room_id').eq('id', gameId).single()
    const roomId = (gameRow as { room_id: string } | null)?.room_id
    if (roomId) {
      await admin
        .from('rooms')
