...
import { selectWordEntry, toWordSet, customWordSet } from '@/lib/game-engine/word-selection'
import { IMPOSTOR_WORDS } from '@/data/impostor-words'
...
export const useLocalGame = create<LocalGameState>()((set, get) => ({
  ...
  start: (settings) => {
    ...
    try {
      ...
      usePreferences.getState().rememberWord(words.sourceId)
      usePreferences.getState().setLastSettings(settings)
      usePreferences.getState().setLocalPlayerNames(players)
...
function resolveLocalWords(settings: GameSettings) {
  ...
  const excludeIds = usePreferences.getState().wordHistory
  const filters = {
