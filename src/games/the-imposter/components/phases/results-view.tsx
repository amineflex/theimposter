...
import { GameResult, type ResultPlayerView } from '@/games/the-imposter/components/game-result'
...
import { t } from '@/i18n'
import type { GamePlayerRow } from '@/games/the-imposter/types/db'
 ...
 */
export function ResultsView({ room }: { room: ImposterRoom }) {
  const { play } = useSound()
  const [roles, setRoles] = React.useState<GamePlayerRow[] | null>(null)
  const [replaying, setReplaying] = React.useState(false)
  const [reopening, setReopening] = React.useState(false)
  const game = room.game
  const wordHistory = useImposterPreferences((state) => state.wordHistory)
  const rememberWord = useImposterPreferences((state) => state.rememberWord)
