...
import { ResultBurst, CountIn } from '@/flexgames/ui/result-burst'
import { Shape, ShapeRow } from '@/flexgames/ui/decor'
import { VoteCard } from '@/flexgames/ui/vote-card'
import { GameResult } from '@/games/the-imposter/components/game-result'
import { SettingsPanel } from '@/games/the-imposter/components/settings-panel'
import { BigStepper } from '@/flexgames/ui/big-stepper'
import { DiscussionBoard } from '@/games/the-imposter/components/discussion-board'
...
import { useLocalCountdown } from '@/flexgames/ui/use-countdown'
import { MIN_PLAYERS, MAX_PLAYERS } from '@/games/the-imposter/engine/types'
import { phaseDuration, reconcileSettings } from '@/games/the-imposter/engine/engine'
import { validateSettings } from '@/games/the-imposter/engine/roles'
import { t } from '@/i18n'
import { AVATAR_KEYS } from '@/flexgames/players/avatars'
...
function SetupStep({ onStart }: { onStart: () => void }) {
  ...
  const savedNames = useImposterPreferences((state) => state.localPlayerNames)
  const lastSettings = useImposterPreferences((state) => state.lastSettings)
